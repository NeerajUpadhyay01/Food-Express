  ...
  fetch(`/get-my-cart/${userId}`)
    ...
    .then((userCartList) => {
      ...
      userCartList.forEach((item) => {
        ...
        templateClone.querySelector(".basketFoodItemName").textContent =
          item.itemName;
        const priceElement = templateClone.querySelector(
          ".basketFoodItemPrice"
        );
      ...
      if (response.ok) {
        response.json().then((updatedCart) => {
          const updatedItem = updatedCart.find(
            (cart) => cart.cartItemId === item.cartItemId
          );
          if (updatedItem) {
...
async function loadOrderSummary(userId) {
  try {
    ...
    document.getElementById(
      "totalAmount"
    ).textContent = `₹${orderSummary.totalAmount.toFixed(2)}`;
    document.getElementById(
      "couponDiscount"
    ).textContent = `₹${orderSummary.couponDiscount.toFixed(2)}`;
    document.getElementById("tax").textContent = `₹${orderSummary.tax.toFixed(
      2
    )}`;
    document.getElementById(
      "savings"
    ).textContent = `₹${orderSummary.savings.toFixed(2)}`;
    document.getElementById(
      "finalAmount"
    ).textContent = `₹${orderSummary.finalAmount.toFixed(2)}`;
  } catch (error) {
