  ...
  try {
    ...
    for (const order of orders) {
      ...
      orderClone.querySelector(".order-status").textContent =
        order.deliveryStatus === "completed" ? "Completed" : "Pending";
      ...
      const orderProductsContainer =
        orderClone.querySelector(".order-products");
      let flag = false;
      let trackResponse;

      for (const item of order.orderItems) {
        const itemClone = orderItemTemplate.content.cloneNode(true);
        itemClone.getElementById(
          "singleFoodItem"
        ).href = `single-food-page?itemId=${item.itemId}`;
        ...
        itemClone.querySelector(".product-quantity").textContent =
          item.quantity;
        ...
        if (order.deliveryStatus === "pickedup" && !flag) {
          ...
          try {
            trackResponse = await fetch(
              `/check-live-tracking-con/${order.orderId}`
            );
            if (trackResponse.ok) {
              flag = true;
            }
            ...
            console.error(
              "Failed to check live tracking condition:",
              trackError
            );
          }
        }
        ...
        if (order.deliveryStatus === "completed") {
          const alreadyReviewed = await isAlreadyReviewed(
            userId,
            order.orderId,
            item.restaurantId
      ...
      if (flag) {
        ...
        orderProductsContainer.parentNode.insertBefore(
          trackButton,
          orderProductsContainer.nextSibling
        );
            ...
            if (defaultAddress) {
              ...
              if (coordinates.latitude && coordinates.longitude) {
                ...
                const partnerLocation = {
                  lat: parseFloat(partner[0][9]),
                  lng: parseFloat(partner[0][8]),
                };

                console.log(customerLocation, partnerLocation);
                document.getElementById("live-tracking-popup").style.display =
                  "block";
                document.getElementById("addressMapTrack").style.display =
                  "block";
                ...
                console.error(
                  "Failed to fetch coordinates for the default address."
                );
              }
            } else {
...
async function getCoordinates(address) {
  ...
  try {
    ...
    if (data.status === "OK") {
      ...
      console.error(
        "Error fetching data for address:",
        fullAddress,
        data.status
      );
    ...
    function (response, status) {
      if (status === "OK") {
        ...
        document.getElementById(
          "eta"
        ).innerHTML = `<strong>Estimated Time of Arrival (ETA):</strong> ${duration}`;
      } else {
        document.getElementById("eta").innerHTML =
          "<strong>Estimated Time of Arrival (ETA):</strong> Unable to calculate";
      }
    }
  );
}
